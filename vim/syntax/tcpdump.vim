...
" IPv4 Address
syn match tcpIp4Address	/\(\d\{1,3}\.\)\{3}\d\{1,3}\%(\.domain\)\?/
syn match tcpIp4Address	/localhost\.\?/
syn match tcpIp6Address	/\<\x\{4,\}:[:0-9a-fA-F]\+\>\%(\/\d\+\)\?\%(\.domain\)\?\%(\.[0-9]\+\)\?\|::1/
" FQDN
syn match tcpFQDN	/\<\([a-zA-Z0-9\-]\{1,63}\.\)\+[a-zA-Z0-9]\{1,63}\>/
...
" Keywords
syn keyword keywords	Flags CNAME AAAA A
...
hi def link tcpIp6Address	Type
...
hi def link keywords	Conditional
