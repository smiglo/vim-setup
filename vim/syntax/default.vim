...
syn match logTime          /\<\%([1-2]\d\{3\}\)\%(0[1-9]\|1[0-2]\)\%(0[1-9]\|[1-2]\d\|3[0-1]\)\%(-\%([0-1]\d\|2[0-3]\)\%([0-5]\d\)\{2\}\)\{0,1\}\>/
syn match logTime          /\<\(Jan\|Feb\|Mar\|Apr\|Jun\|Jul\|Aug\|Sep\|Oct\|Nov\|Dec\)\> \d\+ \d\d:\d\d:\d\d/
syn match logTime          /\[\{0,1\}\<\d\{4\}-\d\d-\d\d\>\%( \d\d:\d\d:\d\d\)\{0,1\}\]\{0,1\}/
syn match logTime          /\<\%(\d\+-\)\{0,1\}\d\{1,2\}:\d\d:\d\d[:.]\d\+\>/
